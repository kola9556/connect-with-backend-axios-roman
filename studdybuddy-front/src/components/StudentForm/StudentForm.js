...
const StudentForm = () => {
  ...
  const onSubmit = ({ name, group, age }) => {
    api
      .post(endpoints.users, { name, group, age: parseInt(age, 10), grades: scores })
  ...
  const handleAddGrade = () => {
    if (scoreInputValue.match(pattern)) {
      setScores([...scores, parseInt(scoreInputValue)]);
    } else {
      alert("You must use only number from 1 to 6!");
    }
    setScoreInputValue("");
  };
